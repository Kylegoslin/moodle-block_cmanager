...
function request_new_mod_email_user($uid, $current_mod_info){
		...
		$user_email_message = $DB->get_record('cmanager_config', array('varname'=>'requestnewmoduleuser'));	
...
function sendEmailToAddress($to, $subject, $text){
	...
	$headers = '';
	$msg = '';
	$headers .= "From: $emailSender <$emailSender>" . "\n";
	$headers .= "Reply-To: $emailSender <$emailSender>\r\n"; 
  	$headers .= "Return-Path: $emailSender <$emailSender>\r\n"; 
...
function email_comment_to_user($message, $uid, $mid, $current_mod_info){
	...
	global $USER, $CFG, $emailSender, $DB;
		...
		$commentForUser = $DB->get_field('cmanager_config', 'value',array('varname'=>'commentemailuser') , IGNORE_MULTIPLE);	    
...
function email_comment_to_admin($message, $mid, $current_mod_info) {
    ...
    // Get each admin email
 	$adminEmailAddresses = $DB->get_recordset_select('cmanager_config', "varname = 'admin_email'");


	// Comment for admin
	$commentForAdmin = $DB->get_field('cmanager_config', 'value',array('varname'=>'commentemailadmin') , IGNORE_MULTIPLE);
...
global $USER, $CFG, $emailSender, $DB;
  ...
  $teacher_ids = '';
	...
	".get_string('emailSubj_mailSent1','block_cmanager').":  ".get_string('emailSubj_mailSent2','block_cmanager')."
