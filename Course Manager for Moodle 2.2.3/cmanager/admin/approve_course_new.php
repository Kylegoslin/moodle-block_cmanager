...
if(isset($_GET['id'])){
	...
	$timestamp_startdate = $DB->get_field('cmanager_config', 'value', array('varname'=>'startdate'), IGNORE_MULTIPLE);
	$new_course->startdate = $timestamp_startdate;


	
	//is course mode enabled (page 1 optional dropdown)
	$mode = $DB->get_field('cmanager_config', 'value', array('varname'=>'page1_field3status'));

	// what naming mode is operating
	$naming = $DB->get_field('cmanager_config', 'value', array('varname'=>'naming'), IGNORE_MULTIPLE);
 	
	//what short naming format is operating
	$snaming = $DB->get_field('cmanager_config', 'value', array('varname'=>'snaming'), IGNORE_MULTIPLE);
	...
	if(isset($rec->modkey)){
	 ...
	 $DB->update_record('cmanager_records', $updatedRecord);
