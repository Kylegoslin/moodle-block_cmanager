 ...
 * --------------------------------------------------------------------------
 */
       $formId = $DB->get_field_select('cmanager_config', 'value', "varname = 'current_active_form_id'");
		...
		$formFields = $DB->get_records('cmanager_formfields',array('formid'=>$formId));
...
function createDropdown($leftText, $id, $form, $fieldName, $selectedValue){
	...
	global $DB;
	 ...
	 $selectQuery = "fieldid = '$id'";
	 $field3Items = $DB->get_recordset_select('cmanager_formfields_data', $select=$selectQuery, $sort='', $fields='*', 
                              $limitfrom='', $limitnum='');
		  foreach($field3Items as $item){
		  	         $value = $item['value'];
					 if($value != ''){
