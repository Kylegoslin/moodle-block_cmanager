...
if(isset($_GET['id'])){
	...
	$formId = $_GET['id'];
	$current_record =  $DB->get_record('cmanager_config', array('id'=>$formId));
...
if(isset($_GET['t']) && isset($_GET['del'])){
    ...
    if($_GET['t'] == 'drop'){
         ...
         // $delId = $_GET['del'];
        //  $DB->delete_records('cmanager_formfields', "id = $delId"); 
		  
		  // Delete the data records
    	  //$DB->delete_records('cmanager_formfields_data', "fieldid = $delId"); 
		  
		  /*
		  // Reorder the rest of the fields
		  
		 $selectQuery = "";
		 // $positionItems = $DB->get_recordset_select('cmanager_formfields', $select=$selectQuery, $sort='position ASC', $fields='*', $limitfrom='', $limitnum='');
			$positionItems = $DB->get_recordset_select('cmanager_formfields', $select=$selectQuery);
		...
		*/
		...
		$itemid = $_GET['del'];
		$fieldid = $_GET['fid'];
		$DB->delete_records('cmanager_formfields_data', array('fieldid'=>$fieldid,'id'=>$itemid)); 
	}
...
if(isset($_GET['del'])){
    ...
    $DB->delete_records_select('cmanager_formfields', "id = $delId"); 	
	
	//Update the position numbers
	$selectQuery = "SELECT * FROM mdl_cmanager_formfields WHERE formid = $formid order by id ASC";
...
if(isset($_GET['up'])){
	...
	$currentRecord = $DB->get_record('cmanager_formfields', array('id'=>$currentId), $fields='*', IGNORE_MULTIPLE);
	$currentPosition = $currentRecord->position;
	
	$higherpos = $currentPosition-1;
    $higherRecord = $DB->get_record('cmanager_formfields', array('position'=>$higherpos), $fields='*', IGNORE_MULTIPLE);
	...
	$DB->update_record('cmanager_formfields', $dataobject2);
...
if ($mform->is_cancelled()){
    ...
    $mform->display();
   	echo $OUTPUT->footer();
		...
		$numberOfFields = $DB->count_records('cmanager_formfields', array('formid'=>$formId));
		echo '<script>numberOfFields = '.$numberOfFields.';</script>';
					  
	   //$formFields = $DB->get_records('cmanager_formfields', 'formid', $formId, $sort='position ASC', $fields='*', $limitfrom='', $limitnum='');
		$formFields = $DB->get_records('cmanager_formfields', array('formid'=>$formId), 'position ASC');
