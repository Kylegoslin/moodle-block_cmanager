...
class courserequest_form extends moodleform {
	...
	$pendingList = $DB->get_recordset_select('cmanager_records', $select=$selectQuery);
  ...
  function goToPage(){
  	...
  	window.location = 'cmanager_admin.php?view=history&p=' + page.value;
        ...
        $numberOfRecords = $DB->count_records_sql("SELECT count(id) FROM " . $CFG->prefix ."cmanager_records WHERE status = 'COMPLETE' OR status = 'REQUEST DENIED'");
  ...
  //  trimmed down in time.
	...
	$pendingList = $DB->get_records_sql("SELECT * FROM ". $CFG->prefix ."cmanager_records WHERE status = 'COMPLETE' OR status = 'REQUEST DENIED' order by id desc LIMIT $fromLimit, $toLimit");
   ...
   foreach($pendingList as $rec){
			...
			$latestComment = '';
			$currentModId = $rec->id;
		 	$modRecords = $DB->get_records('cmanager_comments', array('instanceid' =>$currentModId));
