...
if(isset($_GET['t']) && isset($_GET['del'])){
    ...
    if($_GET['t'] == 'drop'){
          ...
          $delId = $_GET['del'];
          delete_records_select('block_cmanager_formfields', "id = $delId"); 
		  
		  // Delete the data records
    	  delete_records_select('block_cmanager_form_data', "fieldid = $delId"); 
		  
		  
		  // Reorder the rest of the fields
		  //Update the position numbers
		 $selectQuery = "";
		 $positionItems = get_recordset_select('block_cmanager_formfields', $select=$selectQuery, $sort='position ASC', $fields='*', 
	                              $limitfrom='', $limitnum='');
							  ...
							  foreach($positionItems as $item){
								...
								update_record('block_cmanager_formfields', $dataobject);
		...
		$itemid = $_GET['del'];
		$fieldid = $_GET['fid'];
		delete_records_select('block_cmanager_form_data', "fieldid = $fieldid AND id=$itemid"); 
	}
...
$currentid = $_GET['up'];
	...
	$query = "SELECT * FROM mdl_block_cmanager_formfields WHERE id = $currentid";
    ...
    $query = "SELECT * FROM mdl_block_cmanager_formfields WHERE position = $higherpos";
	...
	update_record('block_cmanager_formfields', $dataobject2);
...
//this branch is where you process validated data.
    print_header_simple($streditinga, '',
		...
		$numberOfFields = count_records('block_cmanager_formfields', 'formid', $formId);
	  
							  
	    $formFields = get_records('block_cmanager_formfields', 'formid', $formId, $sort='position ASC', $fields='*', $limitfrom='', $limitnum='');
